...
class DCMHT(nn.Module):
    ...
    def __init__(self, 
        ...
        # self.hash_cls = nn.Linear(bits, numclass).to(1)
        self.hash_cls = nn.Linear(128, 24).to(1)
        self.ratio = 0.3
    ...
    def BNI(self,x):
        ...
        num_b=batch_size*l
        x=x.cuda(1)/(1-self.ratio)
        if num_b%2!=0:
            l=l-1
            num_b=batch_size*l
        noise_b=torch.from_numpy(np.random.permutation(int(num_b/2)*[-1]+int(num_b/2)*[1])).float().view(batch_size,-1).cuda(1)
        for i in range(batch_size):
            idx = (torch.from_numpy(np.random.permutation(x.size(1)))[:l]).cuda(1)
            noise_b = noise_b.to (x).cuda(1)
            x[i,idx] =noise_b[i,:].cuda(1)
        return x

    def forward(self, image, text):
        i1 = self.encode_image(image)
        I_hat_hash = self.hash_cls(self.BNI(i1)).to(1)
        t1 = self.encode_text(text)
        T_hat_hash = self.hash_cls(self.BNI(i1)).to(1)
        return i1, I_hat_hash, t1, T_hat_hash
